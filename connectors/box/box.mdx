...
6. **Complete collaborator lists:** An item's collaborators are read page by page to the end. If a page can't be read, the item keeps the access it already has in PipesHub and the next sync fills in the rest.
...
- Until **Manage users** is fixed, every run is a full sync, which is quick because no files can be listed. The first run after the fix syncs everything.
...
- When a file can't be opened because Box keeps rate-limiting, the message says how long Box asked to wait
