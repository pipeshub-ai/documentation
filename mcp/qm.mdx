...
| `PIPESHUB_BASE_URL is not a valid URL. It holds what looks like a token…` | The two keychain entries are swapped. The CLI never prints the value | Put the PAT under `PIPESHUB_TOKEN` and the origin under `PIPESHUB_BASE_URL` |
