...
<Note>
  Sharing never moves a file. A file or folder owned by someone in your Box enterprise stays in its owner's drive, in its place in the owner's folders, and the colleagues it is shared with see it under **Shared with me**. Files and folders owned by someone outside your enterprise and shared with a colleague aren't brought into PipesHub: Box's API answers "not found" when PipesHub asks for such an item as the colleague, so both full and incremental syncs skip those shares.
</Note>
