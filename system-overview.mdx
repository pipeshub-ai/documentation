...
Stores vector embeddings for semantic search capabilities. Enables fast similarity search and powers the platform's AI-driven search functionality. Selected with `VECTOR_DB_TYPE`; Qdrant, OpenSearch, and Redis are supported.

### Graph database (Neo4j by default)
Manages relationships between entities and enables complex graph-based queries. Useful for understanding connections between documents, users, and metadata. Selected with `DATA_STORE`: `neo4j` (the default written by `install.sh`, the Helm chart, and `env.template`) or `arangodb`.

Services reach the graph through the `IGraphDBProvider` abstraction rather than a driver directly, so the same feature runs on whichever backend an instance was installed with.
