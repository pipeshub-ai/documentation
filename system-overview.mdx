...
Stores vector embeddings for semantic search capabilities. Enables fast similarity search and powers the platform's AI-driven search functionality. Qdrant is what the installer deploys. `VECTOR_DB_TYPE` selects the provider and defaults to `qdrant`.
