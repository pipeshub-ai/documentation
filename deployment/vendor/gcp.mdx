...
- **Graph DB (ArangoDB or Neo4j)**: Multi-model / graph database for relationships and complex queries — chosen when you run the installer
...
- **etcd** and **Kafka** (optional): Used instead of Redis for the KV store and message broker respectively, if selected during installation ("full" deployment)
...
- **Storage**: 100 GB SSD or higher (recommended)
  - PipesHub uses multiple databases (MongoDB, graph DB, Qdrant, Redis, and optionally etcd/Kafka)
...
# Databases (container name depends on what you selected during install)
sudo docker compose -p pipeshub-ai logs -f mongodb
sudo docker compose -p pipeshub-ai logs -f arango   # if graph DB = ArangoDB
sudo docker compose -p pipeshub-ai logs -f neo4j    # if graph DB = Neo4j
...
<Note>
Container names below (`mongodb`, `arango`/`neo4j`, `qdrant`, `redis`, `etcd`) match the fixed `container_name` values in the unified `docker-compose.yml` — they are **not** prefixed with the project name and have no `-1` suffix. The `arango`/`etcd`/`kafka-1` containers only exist if you selected ArangoDB / etcd / Kafka during installation; if you're on the defaults (Neo4j graph DB, Redis broker and KV store), skip those commands or substitute `neo4j` for `arango`.
</Note>
...
# Backup ArangoDB using arangodump (only if graph DB = ArangoDB)
sudo docker exec arango arangodump \
  --output-directory /tmp/arango-backup \
  --overwrite true \
  --server.password="${ARANGO_PASSWORD}"
sudo docker cp arango:/tmp/arango-backup \
  ~/pipeshub-backups/arango-backup-${BACKUP_DATE}
sudo docker exec arango rm -rf /tmp/arango-backup

# Backup Neo4j using neo4j-admin (only if graph DB = Neo4j)
sudo docker exec neo4j neo4j-admin database dump neo4j --to-path=/tmp/neo4j-backup
sudo docker cp neo4j:/tmp/neo4j-backup \
  ~/pipeshub-backups/neo4j-backup-${BACKUP_DATE}
sudo docker exec neo4j rm -rf /tmp/neo4j-backup

# Backup Qdrant using snapshot API
sudo docker exec qdrant \
  curl -X POST 'http://localhost:6333/snapshots' \
  -H 'Content-Type: application/json'
# Copy the snapshot file (check Qdrant logs for snapshot path)
sudo docker cp qdrant:/qdrant/storage/snapshots \
...
# Backup etcd using snapshot (only if KV store = etcd)
sudo docker exec etcd \
  etcdctl snapshot save /tmp/etcd-backup.db \
  --endpoints=http://localhost:2379
sudo docker cp etcd:/tmp/etcd-backup.db \
  ~/pipeshub-backups/etcd-backup-${BACKUP_DATE}.db
sudo docker exec etcd rm /tmp/etcd-backup.db
...
# (drop the -v flags for volumes you didn't provision, e.g. arango_data if you use Neo4j)
sudo docker run --rm \
...
# Backup all volumes (drop any -v flags for volumes you didn't provision,
# e.g. arango_data if you use Neo4j, or etcd_data if you use the Redis KV store)
sudo docker run --rm \
  -v pipeshub-ai_mongodb_data:/data/mongodb \
  -v pipeshub-ai_arango_data:/data/arango \
  -v pipeshub-ai_neo4j_data:/data/neo4j \
...
# Start only the database containers (drop names for components you don't use)
sudo docker compose -p pipeshub-ai up -d mongodb arango qdrant redis etcd
...
# Restore ArangoDB (only if graph DB = ArangoDB)
sudo docker cp ~/pipeshub-backups/arango-backup-${BACKUP_DATE} \
  arango:/tmp/arango-backup
sudo docker exec arango arangorestore \
  --input-directory /tmp/arango-backup \
  --server.password="${ARANGO_PASSWORD}" \
  --overwrite true
sudo docker exec arango rm -rf /tmp/arango-backup

# Restore Neo4j (only if graph DB = Neo4j)
sudo docker cp ~/pipeshub-backups/neo4j-backup-${BACKUP_DATE} \
  neo4j:/tmp/neo4j-backup
sudo docker exec neo4j neo4j-admin database load neo4j \
  --from-path=/tmp/neo4j-backup --overwrite-destination=true
sudo docker exec neo4j rm -rf /tmp/neo4j-backup
...
# Restore etcd (only if KV store = etcd)
sudo docker cp ~/pipeshub-backups/etcd-backup-${BACKUP_DATE}.db \
  etcd:/tmp/etcd-backup.db
sudo docker exec etcd etcdctl snapshot restore /tmp/etcd-backup.db
...
# Remove old volumes (drop lines for volumes you never provisioned)
...
# Test database connections (skip lines for components you don't use)
sudo docker exec mongodb mongosh --eval "db.adminCommand('ping')"
sudo docker exec arango curl -s http://localhost:8529/_api/version
