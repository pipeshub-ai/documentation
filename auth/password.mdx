...
## Password Authentication
    ...
    A wrong password and an email with no account get the same message: *"The email or password is incorrect. Check both and try again, or use Forgot password to set a new password."*

    After five wrong passwords in a row, the account is locked for 24 hours and the owner is emailed. While it is locked, even the right password gets the same "incorrect" message. The account unlocks by itself 24 hours after the lock was applied. See [How sign-in protects accounts](/auth#how-sign-in-protects-accounts).

    ### Reset Password
    For resetting the password, user can go to the profile page and click on the change password option.

    <img src="/images/auth/reset_password.png" alt="Reset password" width="90%" />
