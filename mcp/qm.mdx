...
You do not need bun, and you do not need to build the CLI from source. Install [`@pipeshub-ai/mcp@2.3.2`](https://www.npmjs.com/package/@pipeshub-ai/mcp) or later — 2.3.1 still writes a `sandbox/Dockerfile` that Sprites never boot.
...
```bash
npm install -g @pipeshub-ai/mcp@2.3.2
pipeshub init-qm .
```
...
Re-running is safe: existing files are kept. Do not re-run 2.3.1 after deleting the file, or it will write it again.
...
That Sprite install is unpinned on purpose. The operator machine is pinned to 2.3.2 so `init-qm` skips the unused Dockerfile; the Sprite never runs that Dockerfile ([qm#272](https://github.com/yc-software/qm/issues/272)), so pinning the first-run command would only freeze the CLI that actually executes. Leave it unpinned so a Sprite picks up patches without a skill edit.
