...
<div className="not-prose mt-10">
  <div className="p-6 border border-gray-200 dark:border-gray-700 rounded-xl bg-blue-50 dark:bg-gray-800">
    <div className="flex items-center gap-3 mb-4">
      ...
      />
      <div className="text-xl font-semibold text-gray-900 dark:text-white">Microsoft Teams actions</div>
    </div>
    <p className="text-base text-gray-700 dark:text-gray-300 mt-0 mb-5">Team channels, chats, meetings, and calendar via Microsoft Graph</p>
...
<Info>
  Channel message search scans recent messages per channel (bounded by the tool’s parameters). Ask for a specific team or channel when possible so results stay focused. If none of the channels could be read, the search reports the error instead of "no matches".
</Info>

If Microsoft refuses to list your directory (for example, because a permission hasn't been granted), the tools that look people up say so and name the next step, instead of reporting that nobody by that name exists. User lists follow every page, so a limit above 100 returns up to that many people. Errors from Microsoft Graph keep Graph's own message and add what to do next.
...
Ask to “DM” or “chat with” a person by display name, email, or UPN. The Microsoft Teams action uses `send_user_message`. It looks the person up across your whole directory and sends only on an **exact** match of display name, email, sign-in name (UPN) or ID. If the name only partly matches (for example "Sam" when the directory has "Samantha Lee"), nothing is sent: the agent gets the closest matches and asks you to confirm with an email or ID. Reading a chat with a person uses the same matching.
...
Use natural language with a time range. The agent can call `get_meetings`, `search_calendar_events_in_range`, `create_event`, or `create_channel_meeting` depending on whether the meeting is personal or tied to a channel. For a recurring meeting, say how it repeats: daily, weekly on certain days, monthly on a date (the 15th) or on a weekday (the first Monday), or yearly. A repeat pattern Microsoft doesn't accept is refused with a message saying what to send, rather than quietly becoming a daily meeting.
