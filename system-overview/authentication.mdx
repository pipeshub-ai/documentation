...
2. **Authentication Steps**
   ...
   - The method must be one the organization allows for the **current** step. Any other method is refused with `400` before any password or code is checked
   ...
   - Every step must be completed by the same account. A later step completed for a different account (for example a different Google, Microsoft, OAuth or SAML identity) is refused, and no account is created for it
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| method | string | Yes | Authentication method (password, otp, google, microsoft, azureAd, oauth). `samlSso` is not handled here; see the note below |
| credentials | object | Yes | Credentials object for the specified method |

<Note>
  SAML uses browser redirects, so it has its own sign-in route, `GET /api/v1/saml/signIn`. Sending `"method": "samlSso"` to this endpoint returns `400` with *"Single sign-on (SAML) can't be completed with this request. On the sign-in page, choose your organisation's single sign-on option. Apps calling the API directly should send the browser to /api/v1/saml/signIn instead."*
</Note>
...
Refusals are worded the same way whether or not the email has an account, and take about as long, so the endpoint can't be used to find out which emails are real.

| Situation | Status | Message |
|---|---|---|
| Wrong password, unknown email, no password set, password missing from the request, or a locked account | `400` | "The email or password is incorrect. Check both and try again, or use Forgot password to set a new password." |
| Wrong, expired or already-used code, no code requested, unknown email, or a locked account | `401` | "That sign-in code isn't right. Check the most recent code in your email, or request a new one." |
| Method not allowed for the current step | `400` | "That sign-in method isn't turned on for this step. Go back to the sign-in page and use one of the options it shows, or ask your admin which sign-in methods are enabled." |
| `samlSso` sent to this endpoint | `400` | See the note in the Request tab |
| Step completed with a different account than the step before it | `401` | "This step was completed with a different account than the step before it. Start again from the sign-in page and use the same account for every step." |
| No `x-session-token` header | `401` | "Invalid session token" |
| Sign-in session expired or unknown (a sign-in session expires an hour after it starts or after its last completed step) | `401` | "Invalid session" |

After five wrong passwords or codes in a row, the account is locked for 24 hours and the owner is emailed. During the lock, even correct credentials get the same answer as a wrong password or code. An expired code now gets the `401` wrong-code answer above, not `410`.
</Tab>
</Tabs>
</Accordion>
...
The body is this plain-text message:
...
The answer is the same `200` and message for every email: an account, an unknown email, a locked account, or an email that couldn't be sent. A code is created and emailed only when the account exists and isn't locked, and it is sent in the background, so the answer doesn't wait for the mail server. A failed send is logged on the server. This endpoint no longer returns `404` or `403`.

A code is valid for 10 minutes and can be used once.
</Tab>
</Tabs>
</Accordion>
...
- `samlSso` can only be used in a one-step configuration. A configuration with more than one step that includes `samlSso` in any step is refused with `400`: "SAML single sign-on can't be combined with other sign-in steps yet. Use SAML on its own as a one-step sign-in, or remove it from the policy." One step that offers SAML alongside other methods (for example SAML or password) is allowed
...
Errors use the API's standard shape:

```json
{
  "error": {
    "code": "HTTP_BAD_REQUEST",
    "message": "The email or password is incorrect. Check both and try again, or use Forgot password to set a new password.",
    "requestId": "…"
  }
}
```

`requestId` is included when the request has one. The codes you'll see from these endpoints:

- `HTTP_BAD_REQUEST` - A request the service refuses, including a wrong password (see the `/authenticate` error table)
- `HTTP_UNAUTHORIZED` - A missing, invalid or expired sign-in token or sign-in session; a failed sign-in-code check (wrong, expired or already-used code, no code requested, unknown email, or a locked account); or a sign-in step completed with a different account. A wrong current password on the change-password endpoint is also `HTTP_UNAUTHORIZED` ("Current password is incorrect."). On `/authenticate`, a wrong password is `HTTP_BAD_REQUEST` instead
- `HTTP_FORBIDDEN` - Not allowed, for example an OAuth token without the required scope
- `HTTP_NOT_FOUND` - Resource not found
- `VALIDATION_ERROR` - The request body or parameters don't match the expected format
- `INTERNAL_ERROR` - A failure on PipesHub's side; the message doesn't describe internals

There is no separate code for a locked account. A locked account gets the same answer as a wrong password (`HTTP_BAD_REQUEST`) or a wrong sign-in code (`HTTP_UNAUTHORIZED`), so that sign-in doesn't reveal whether an account exists.
...
3. **Account Blocking:** Users are automatically blocked for 24 hours after 5 failed login attempts, and the owner is emailed. Sign-in answers a blocked account the same way as a wrong password or code
