...
- **Failed reads** — if a space's permissions, a group's members, or a page's restrictions can't be read for a temporary reason, PipesHub keeps what it already has. A page whose restrictions can't be read is skipped for that sync and retried on the next one, and the sync doesn't move past it. A failed read never opens a page to its whole space and never removes access
