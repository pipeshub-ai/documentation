...
If a single issue or merge request can't be read or saved, the rest of the project still syncs.
The checkpoint is saved just before the earliest failed item, so the next run asks for it again
