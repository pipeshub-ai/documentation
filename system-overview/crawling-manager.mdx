...
Finished runs (`completed`) aren't reported. If a connector's only jobs have finished, this endpoint answers `404` "No scheduled job found for this connector".
...
The list holds, for each connector, up to its 10 most recent queued, delayed, active or failed jobs, followed by every paused schedule in the organization. Finished runs aren't listed. Each entry has the same fields as **Get Job Status**; this example shortens `scheduleConfig`.
