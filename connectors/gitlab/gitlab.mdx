...
The OAuth token is refreshed only when GitLab actually answers **401 Unauthorized**, and only once
when several requests fail together, so refresh tokens aren't used up needlessly.
