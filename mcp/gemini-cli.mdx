...
<Tip>
  Coding agents: read [For coding agents](/for-agents) first. If they have no PipesHub yet, that page has the local Docker demo (do not scaffold LangChain). MCP config is not enough — also install the skill into **this** repo (`npx skills add pipeshub-ai/mcp-server`) and append the `AGENTS.md` snippet on that page.
</Tip>
