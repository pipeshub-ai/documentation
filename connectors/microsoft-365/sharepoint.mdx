...
<Accordion title="Is it possible to index only specific SharePoint sites?">

Yes. When creating a SharePoint connector, the admin has the option to apply filters and select specific SharePoint sites for indexing. This allows you to limit the scope of indexed content to only the sites that are relevant to your organization's needs.
...
1. **Create separate connector instances** — You can create multiple instances of the same connector type. For example, create a dedicated SharePoint connector instance for each specific site you want to index.
...
3. **Navigate to specific folders** — Go to **All Records** in the navigation bar, then browse to the specific files or folders you want to index within each SharePoint site.
