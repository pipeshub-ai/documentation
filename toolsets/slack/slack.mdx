...
<div className="not-prose mt-10">
  <div className="p-6 border border-gray-200 dark:border-gray-700 rounded-xl bg-blue-50 dark:bg-gray-800">
    <div className="flex items-center gap-3 mb-4">
      ...
      />
      <div className="text-xl font-semibold text-gray-900 dark:text-white">Slack Toolset</div>
    </div>
    <p className="text-base text-gray-700 dark:text-gray-300 mt-0 mb-5">Team messaging, channels, and collaboration</p>
...
Ask to "send a DM to John about the meeting" or "directly message sarah@company.com". The agent can send private messages to any user. Giving the person's email is the surest way to reach the right person. An email address nobody in the workspace has is refused rather than matched to someone with a similar name, and a longer name you type (such as "Joanna") no longer matches a shorter one in the directory (such as "Ann"). If Slack won't let the agent read the directory (a rate limit or a missing scope), the agent says so instead of "User not found".

**Are long lists complete?**  
Listing tools (channel members, users, conversations) read every page Slack returns. If Slack stops partway, the agent is told the list is incomplete rather than being handed part of it as if it were the whole. Errors come back in plain words with what to do next, worded for the user token this toolset signs in with.

**Does the toolset work with Slack token rotation?**  
Yes. If your Slack app has **token rotation** turned on, its sign-in expires about every 12 hours, and PipesHub renews it automatically before then.

- If the agent says *"The Slack sign-in has expired. PipesHub renews it automatically, so try again in a few minutes."*, wait a few minutes and ask again. If it keeps happening, reconnect the Slack toolset in **Settings > Toolsets**.
- If the agent says *"Slack did not accept the saved sign-in."*, Slack has refused the sign-in (for example, it was revoked or the app was reinstalled), and renewing can't fix it. Reconnect the Slack toolset in **Settings > Toolsets**.
