...
4. **Per-meeting enrichment** — for each meeting:
   ...
   - `GET /v2/report/meetings/{uuid}/participants` — attendee list
   - `GET /v2/meetings/{uuid}/transcript` — AI Companion transcript (if enabled)
...
- **Meeting details and recording share URLs** across the account → `meeting:read:meeting:admin`, `cloud_recording:*:admin`
