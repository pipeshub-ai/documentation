...
<Info>
  The Slack bot must be invited to a channel (using `/invite @YourBot`) before it can send messages or read history.
</Info>
...
<div className="text-center">
  <img
    src="/images/toolsets/slack/create-app-dialog.png"
    alt="Create Slack App - Choose From Manifest"
...
5. The manifest JSON will be displayed. Click **"Next"** to proceed
...
<div className="text-center">
  <img
    src="/images/toolsets/slack/bot-token-scopes.png"
...
3. Copy your **Client ID** and **Client Secret** (click "Show" to reveal)

<div className="text-center">
  <img
    src="/images/toolsets/slack/app-credentials.png"
...
<div className="text-center">
  <img
    src="/images/toolsets/slack/slack-oauth-consent.png"
    alt="Slack Authorization"
    className="block mx-auto w-11/12"
  />
</div>
...
Ask your agent to "send a message to #marketing" with your content. The agent supports Slack formatting like `*bold*` and `_italic_`.
...
Simply ask to "send a message and mention @john" or "notify Sarah about this update". The agent will handle mentions automatically.
...
Ask to "schedule a message for tomorrow at 9am" and specify the channel. The agent can schedule messages for future delivery.
...
Ask to "pin that message" or "pin the meeting notes to #general". Pinned messages stay at the top of the channel for easy reference.
...
**Why can't my agent send to a channel?**  
The bot must be invited to the channel first. In Slack, type `/invite @YourBot` in the channel, then the agent can send messages there.
