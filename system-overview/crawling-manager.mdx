...
Two more `400 Bad Request` messages you may see:

- *"Scheduled time is missing or is not a valid date"* — a `once` schedule without a readable top-level `scheduledTime`.
- *"This schedule can't be used: it never produces a run time. Check the cron expression and the timezone name, then try again."* — a cron expression or timezone that the scheduler can't use.

These two refusals leave the connector's existing schedule running unchanged. A schedule sent with `isEnabled: false` is different: it removes the connector's current schedule and then answers `400` "Cannot schedule a disabled job".
...
**Status:** `400 Bad Request` (caller is not an admin)

Message: "Admin access required". Nothing is removed.
