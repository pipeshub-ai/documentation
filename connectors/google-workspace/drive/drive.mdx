...
- A folder under **Folder IDs**, on either connector, or any refused folder on a personal Drive, fails the whole sync run.
...
- **A folder under Folder IDs** is left out of that run's scope; your saved **Folder IDs** list isn't changed. The folder is tried again on every run. Once it can be read, new changes inside it sync again, and a full sync brings in the files it already held. On Workspace, the other synced users try a refused folder too, and the run only fails, and the folder only counts a run, if none of them can read it.
...
Quota and rate-limit errors are different: they still fail the run each time until the quota refills, so no files are dropped. Known permission refusals, and folders that no longer exist, are skipped straight away, as before.
