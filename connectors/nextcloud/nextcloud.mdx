...
4. **Deletion Handling:** Removes deleted files from PipesHub. Deleting a folder removes everything that was inside it too.
...
- **Deletions keep being retried:** A deleted file never changes again, so a deletion that still fails on the 5th sync goes into a list of pending deletions, saved together with the cursor. A file that Nextcloud lists again during that sync (because it was restored or recreated) isn't queued. Every sync retries that list first, before reading new changes, and checks with Nextcloud before deleting:
  ...
  A deletion also leaves the list when nothing from that file or folder is left in PipesHub. If a folder's own record is already gone but files from inside it are still indexed, those files are removed first, and the deletion stays on the list until that succeeds. Until a deletion is applied, the file stays searchable.
