...
**Fastest for a Docker demo:** the [Demo connector](/demo-data) — a fictional company across five systems, no files and no OAuth (first release after 0.8.0). Otherwise, Knowledge Base upload in the UI (no extra mounts): have them drop a few `.md` / `.txt` / PDF files.
...
Call `pipeshub_sources`, then `pipeshub_search` on a term from the files they uploaded — or, with the [Demo connector](/demo-data), on its first question: *"Why was the payment service architecture changed, and how was the decision made?"* (there are no uploaded files in that setup). Back off (e.g. 15s, 30s, 60s, up to several minutes):

| Result | Meaning |
| --- | --- |
| No sources / empty KB | The Demo connector is not enabled (or still syncing), or they have not uploaded or connected a folder. Not a product failure. |
