...
The OneDrive connector uses OAuth 2.0 via Microsoft Graph API with Application Permissions, allowing background access to organizational mailboxes without requiring individual user sign-ins.
...
The `run_sync` method orchestrates the complete synchronization process in four main steps:
...
- Member addition is handled via ADD/UPDATE function
...
5. Returns a `RecordUpdate` item along with permissions
