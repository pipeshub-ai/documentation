...
PipesHub uses email addresses to match Confluence users with their permissions. If a user's email is hidden:
- PipesHub cannot match the user's Confluence account to their PipesHub account.
- This prevents permissions from being synced correctly for that user.
- As a result, the user will not see any Confluence content in PipesHub.
...
<Info>
  This is a **per-user setting** that must be configured by each individual user in your organization. Administrators cannot change this setting for other users. We recommend communicating this requirement to your team during the Confluence connector rollout.
</Info>
