...
Every agent already has a set of **built-in capabilities**. They are attached
automatically, they do not appear under **Workspace settings → Actions**, and there
is nothing to configure — you cannot add or remove them. They are listed here so you
know what an agent can already do before you attach anything.

| Built-in capability | What the agent can do | Tools |
|---------------------|-----------------------|-------|
| Retrieval | Search the organisation's indexed knowledge | 1 |
| Knowledge Hub | Find indexed items by name | 1 |
| Knowledge Graph | Navigate and search content through the graph | 4 |
...
| Code Sandbox | Run Python or TypeScript in an isolated sandbox | 2 |
| Database Sandbox | Run SQL in an ephemeral database | 2 |
| Image Generator | Generate or edit an image from a prompt | 1 |

Two of these are worth knowing about when writing prompts. **Calculator** exists
because language models are unreliable at arithmetic, so an agent that must produce
exact numbers should be told to use it. **Image Generator** calls whichever image
model is configured under **Settings → AI Models**, so it only works once one is set
up.

<Warning>
**Code Sandbox** and **Database Sandbox** execute code, and every agent has them.
Python, TypeScript and SQL run in an isolated sandbox rather than against your
services, but if your security review needs to account for code execution, this is
where it happens.
</Warning>
...
That is **19 catalogue toolsets** carrying 342 tools that you authorise and attach, on top of the 9 built-in capabilities above.
