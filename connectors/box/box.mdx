...
3. **Incremental Detection:** Each folder maintains its own sync cursor, enabling efficient incremental updates.
...
6. **Cursor Management:** After each folder sync, saves the folder's cursor for incremental updates.

#### Incremental Sync

Uses the Box Enterprise Event Stream to efficiently process only changes:
...
7. **Cursor Update:** Saves new stream position after processing all events.
...
- **Schedule Appropriately:** Set sync intervals based on your team's activity patterns
- **Schedule Wisely:** Run scheduled syncs during off-peak hours for large datasets
