...
- **Deletions keep being retried:** A deleted file never changes again, so a deletion that still fails on the 5th sync goes into a list of pending deletions, saved together with the cursor. This only happens when the deletion is that file's latest change on the page; a file restored or recreated later on the same page isn't queued. Every sync retries that list first, before reading new changes, and checks with Nextcloud before deleting:
  ...
  - if Nextcloud can't be checked right now, the deletion stays on the list for the next sync.
