...
- **Projects**: All accessible projects and their code on default branch
...
- Verify you're signing in with a valid GitLab account that has access to the required repositories
...
1. **Code Repository Fetch**: Retrieves the code repository of the project on default branch
...
<Accordion title="Why do I see no issues after authenticating?">
  The most common causes are missing scopes or selecting the wrong GitLab account. Re-run authentication, ensure the PipesHub email and GitLab account email matches.
</Accordion>
