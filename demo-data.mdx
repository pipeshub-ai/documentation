...
Acme Corp is 53 records across five systems, loaded the way real connectors load them, so search, citations and permissions behave exactly as they would with your own data:
...
Hiding changes only what you see. Nothing is deleted, and everyone else keeps their own setting. Records from the demo carry a **Demo** badge wherever they are cited or listed, so a fictional Slack thread never passes for a real one.
...
- **Keep for now** — hides the notice for a week.

To do it by hand, delete the Demo connector from **Workspace → Connectors**. Its records, groups and permissions go with it. If you created Alice and Bob, delete them under **Workspace → Users**. Removed sample accounts can be created again later with the headless setup.
