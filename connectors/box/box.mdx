...
If a page of history can't be read, or its events can't be applied, the replay stops there:

- A temporary error marks the sync incomplete, so the next full sync reads that page again. Pages saved earlier in the run stay saved.
- A 403 or 404 from Box is final and doesn't mark the sync incomplete, so the sync can still finish and switch to incremental runs. Incremental runs don't replay history, so the rest of it waits for a later full sync.
