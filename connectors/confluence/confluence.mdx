...
1. **Space Keys** - Filter by Confluence spaces
   ...
   - **Value**: Comma-separated space keys (e.g., `ENG, MKT, PROD`)
   - **Find space keys**: Check your Confluence URL - `https://yourinstance.atlassian.net/wiki/spaces/SPACEKEY/overview`

2. **Page IDs** - Filter specific pages (includes all child pages automatically)
   - **Operator**: `In` (include) or `Not In` (exclude)
   - **Value**: Comma-separated page IDs (e.g., `88375321, 90341377`)
   - **Find page IDs**: Check page URL - the number after `/pages/` is the page ID
...
3. **Blogpost IDs** - Filter specific blogposts
   ...
   - **Value**: Comma-separated blogpost IDs
   - **Find blogpost IDs**: Check blogpost URL - the number after `/blog/YYYY/MM/DD/` is the blogpost ID
