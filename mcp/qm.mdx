...
Cite `recordId` and `webUrl` when the CLI returns them. Do not invent a source. Exit `6` on `ask` means this response had no citation objects — the answer text is still in the output.
