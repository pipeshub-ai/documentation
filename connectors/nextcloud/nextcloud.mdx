...
- **The limit:** A page is held for at most 5 syncs. On syncs 1 to 4 the cursor stays where it was. On the 5th, the connector moves the cursor on, so one broken file can't hold up later changes, and logs an error. That error doesn't name the file. After that, a failed change is read again only when that file next changes in Nextcloud.

<Warning>
  **A deletion that fails 5 times is not applied.** A deleted file never changes again, so nothing brings the deletion back. The file stays searchable in PipesHub.
</Warning>
