...
<div className="not-prose mt-10">
  <div className="p-6 border border-gray-200 dark:border-gray-700 rounded-xl bg-blue-50 dark:bg-gray-800">
    <div className="flex items-center gap-3 mb-4">
      ...
      />
      <div className="text-xl font-semibold text-gray-900 dark:text-white">Confluence Cloud</div>
    </div>
    <p className="text-base text-gray-700 dark:text-gray-300 mt-0 mb-5">Team workspace and documentation platform</p>
...
<Info>
  This page is for **Confluence Cloud**, the version Atlassian hosts for you. If you run Confluence on your own servers, use the [Confluence Data Center connector](/connectors/confluence/confluence-data-center) instead.
</Info>
...
| `read:confluence-content.permission` | Read content permissions in Confluence |
...
3. Enter:
   ...
   - **Grant Jira user access** — select **Yes** or **No** (see note below).
4. Click **Next** to move to the **Authorize** tab.

<div className="text-center">
  <img
    src="/images/connectors/confluence/connector_add_creds.png"
    alt="Authenticate Instance tab — enter Client ID, Client Secret, and Grant Jira user access"
    className="block mx-auto w-11/12"
  />
</div>

<Info>
  **Grant Jira user access.** Choose **Yes** only if your Atlassian OAuth app includes Jira and you've added the `read:jira-user` scope. PipesHub will request that scope during authorization and may use Jira to resolve user emails when Confluence profiles hide them. Choose **No** if you don't use Jira on this site or haven't added that scope.
</Info>
...
<div className="text-center">
  <img
    ...
    alt="Authenticate Instance tab with API Token method — Base URL, Email, API Token fields"
    className="block mx-auto w-11/12"
  />
</div>
...
<Info>
  This connector works with **Confluence Cloud** only. If you run Confluence on your own servers, use the [Confluence Data Center connector](/connectors/confluence/confluence-data-center).
</Info>
...
- Syncs every attachment of a page or blog post, however many it has. If the attachment list can't be read in full, the attachments already read are kept, and the space's page or blog post checkpoint doesn't move, so the next sync reads that time window again. That includes a temporary failure (such as a timeout or rate limit) on the list's first request, in which case the connector uses the attachments that came with the page or blog post in the search results. If the first request fails for a lasting reason, such as the list not being found, the connector uses those search-result attachments and the checkpoint moves on as usual
...
- Attachments and comments of a restricted page are restricted the same way as the page. When a page becomes restricted, or stops being restricted, its stored attachments and comments (and their replies) follow
- A page restricted to a group PipesHub hasn't synced yet stays restricted; the group's members get access once the group syncs and the page is read again
- If a space's permissions, a group's members, or a page's restrictions can't be read for a temporary reason, PipesHub keeps what it already has and tries again on the next sync. A failed read never opens a page to its whole space and never removes access
