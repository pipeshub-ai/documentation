...
A grounded answer includes citations with `recordId` and `webUrl`. An `ask` with **no citations** means nothing was retrieved. If it says the documents do not contain it, that is often the correct answer. If it asserts facts anyway, do not repeat them. Do not invent a source.
