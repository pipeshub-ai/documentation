...
Syncs only the folders you list, and everything inside them. To find a folder's ID, open the folder in Google Drive: the ID is the last part of the address, `drive.google.com/drive/folders/<FOLDER_ID>`.
...
- **A folder under Folder IDs** is left out of that run's scope, and nothing fails; your saved **Folder IDs** list isn't changed. The folder is tried again on every run. Once it can be read, new changes inside it sync again, and a full sync brings in the files it already held.
