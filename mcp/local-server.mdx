...
- A request that gets no response within 60 seconds is abandoned. The limit covers the wait for PipesHub to start answering, so a long chat answer that is already streaming is not cut off.

When a call fails because PipesHub can't be reached or doesn't answer in time, the tool's error names the address, gives the reason (for example a refused connection or an untrusted certificate), and says what to check. The request's bearer token is removed from any error text before the model sees it.
