...
Yes. If your Slack app has **token rotation** turned on, its sign-in expires about every 12 hours, and PipesHub renews it automatically before then. If an agent reports that the Slack sign-in has expired, try again in a few minutes. If it keeps happening, or the agent says Slack no longer accepts the saved sign-in, reconnect the Slack toolset in **Settings > Toolsets**.
