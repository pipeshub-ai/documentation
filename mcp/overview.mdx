...
<Note>
  **[QM](/mcp/qm) is not an MCP client.** It cannot attach to `/mcp` the way the clients below can. The integration is a `pipeshub` CLI inside the agent sandbox — see [Use with QM](/mcp/qm).
</Note>

## Prerequisites

- A running PipesHub instance (self-hosted or cloud). If they have none yet, follow [Local Docker demo](/for-agents-local-demo) instead of scaffolding LangChain.
...
<Note>
  Most **client setup guides below** (Cursor, Gemini CLI, Claude.ai, LibreChat) connect over OAuth — you'll need an OAuth app for those. [Claude Code](/mcp/claude-code) and [Omnigent](/mcp/omnigent) are the exceptions: they attach with an `Authorization` header, so a personal access token is all you need. Claude Code can also use an OAuth app for shared or admin-managed setups.
...
<CardGroup cols={2}>
  ...
  <Card title="Claude Code" icon="/logo/claude.svg" href="/mcp/claude-code">
    Remote HTTP MCP with a personal access token, or an OAuth app for shared setups.
  </Card>
  ...
  <Card title="Omnigent" icon="/logo/omnigent.svg" href="/mcp/omnigent">
  ...
  <Card title="QM" icon="terminal" href="/mcp/qm">
