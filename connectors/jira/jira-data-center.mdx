...
<div className="not-prose mt-10">
  <div className="p-6 border border-gray-200 dark:border-gray-700 rounded-xl bg-blue-50 dark:bg-gray-800">
    <div className="flex items-center gap-3 mb-4">
      ...
      />
      <div className="text-xl font-semibold text-gray-900 dark:text-white">Jira Data Center</div>
    </div>
    <p className="text-base text-gray-700 dark:text-gray-300 mt-0 mb-5">Self-hosted project management and issue tracking</p>
...
| `GET /rest/auditing/1.0/events` | Jira administrator | The connector cannot tell which issues were deleted, so deleted issues stay in the index. The connector keeps its place, and once the account can read the audit log, every deletion since the last successful read is applied. |

<Note>
  A **temporary** error (a timeout, a server error, rate limiting) is not the same as access being denied. If a project's permissions or a group's members can't be read for that reason, the project or group keeps the access PipesHub already has, and its issues still sync. A failed read never opens content up and never removes anyone's access. The next sync tries again.
</Note>
...
**Project role access doesn't update for groups past the first 1000:**
- Jira Data Center's group list returns at most 1000 groups, and it has no way to page past them. The sync logs say *"More than 1000 groups (Jira reports N): roles that include groups past the first 1000 were left unchanged."*
- The first 1000 groups keep syncing. A project role that includes a group past the first 1000 keeps the members it already has in PipesHub, so nobody loses access. But that role's membership isn't updated: people who join a group past the first 1000 don't gain the role's access in PipesHub, and people who leave it keep the access they had. Roles made only of users, or only of groups in the first 1000, sync as usual.
...
If one issue can't be processed, the other issues in its project still sync. The project's checkpoint waits at that issue, so the next sync tries it again. After five syncs in a row fail on the same issue, the connector moves on without it, logs an error naming the issue, and picks it up again the next time the issue changes.

If Jira rate-limits the issue search, the connector tries the search up to three times, waiting as long as Jira asks each time, but no more than a minute. If Jira is still rate-limiting after the third try, that project is skipped for this sync, and the next sync picks up where it stopped.
