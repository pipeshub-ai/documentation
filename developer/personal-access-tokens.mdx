...
<Warning>
  The admin list response shape is **not** the same as your own token list. It's wrapped in `data`/`pagination`, and each item carries owner fields the self-service list doesn't:
