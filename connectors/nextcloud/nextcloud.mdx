...
- **Deletions keep being retried:** A deleted file never changes again, so a deletion that still fails on the 5th sync goes into a list of pending deletions, saved together with the cursor. A file that Nextcloud lists again during that sync (because it was restored or recreated) isn't queued. Every sync retries that list first, before reading new changes, and looks the file up in Nextcloud by its ID before deleting, so a file or folder that was moved is still found:
  ...
  - if Nextcloud still has it, anywhere in your files, the deletion is dropped and it stays;
  - if the lookup can't be made right now, or its answer can't be read, the connector falls back to the file's saved location in PipesHub. It checks that location only when it is the file's full path: "not found" there, or a different file in its place, applies the deletion, and the same file drops it. If the saved location can't be read, or is only the file's name while the file sits in a folder, the deletion stays on the list for the next sync. A folder whose own record is already gone, while files from inside it are still indexed, always waits for a lookup by ID, because its old location alone can't tell a move from a deletion.
  - if the lookup finds such a folder somewhere else (it was moved), the folder is saved where it is now, and the files inside it are linked back under it.
