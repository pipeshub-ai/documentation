...
2. After the sync completes, the connector saves the latest activity ID as a cursor. If the first sync can't read or save the whole drive, no cursor is saved, and the next sync starts over with a full sync.
...
Click **Save** to store the credentials. PipesHub validates the connection against Nextcloud before persisting. If Nextcloud rejects the app password, the save fails with a message telling you to create a new app password and reconnect.
...
**Symptom:** Saving the connector fails with this message:

> Nextcloud rejected the app password, so nothing could be synced. Create a new app password in Nextcloud (Personal settings > Security > Devices & sessions), then enter it in this connector's settings and save to reconnect.

If the password is revoked after setup, syncs stop, and you get a notification titled **Nextcloud app password rejected** with the same message. Files already synced stay as they are.
...
5. If the password was revoked or expired, create a new one under **Personal settings > Security > Devices & sessions**, enter it in the connector's settings, and save to reconnect
