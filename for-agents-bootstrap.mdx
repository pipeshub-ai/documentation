...
- Connector OAuth stays in a **browser**. For something to search without connecting anything, set `PIPESHUB_DEMO_DATA=1` to load the [Acme Corp demo data](/demo-data); otherwise use a Knowledge Base upload or Local FS.
...
- Index something (the Demo connector if `PIPESHUB_DEMO_DATA=1` was set, otherwise a KB upload or Local FS), then `pipeshub_sources` and `pipeshub_search` with backoff. Empty hits means still indexing, not "PipesHub is broken."
