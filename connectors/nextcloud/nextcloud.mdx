...
If the Nextcloud profile can't be read (for example, during a brief server outage), the sync saves nothing and tries again next time. It never guesses the owner of your files.
...
- **A file or deletion that fails:** If a changed file can't be fetched or saved, or a deletion can't be applied, the cursor stays where it was and the next sync tries that page of changes again. After 5 syncs in a row the connector moves on, so one broken file can't hold up later changes, and it logs an error naming the problem.
...
* **Rate Limiting:** Sends at most 50 requests per second. When Nextcloud answers with a rate limit (HTTP 429) or a temporary server error, the request is retried up to 3 times. Each retry waits as long as Nextcloud's `Retry-After` header asks, up to 60 seconds.
...
**Symptom:** Saving the connector fails with the message *"Nextcloud rejected the app password, so nothing could be synced."* If the password is revoked after setup, syncs stop and you get a notification with the same message. Files already synced stay as they are.
