...
Do **not** invent curls against first-run APIs (`POST /org`, login, PAT create, LLM config). PAT create returns the secret in JSON — unsafe in an agent transcript. The supported mint-to-file path is [Scripted first-run](/for-agents-bootstrap).
