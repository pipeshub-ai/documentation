...
<div className="not-prose mt-10">
  <div className="p-6 border border-gray-200 dark:border-gray-700 rounded-xl bg-blue-50 dark:bg-gray-800">
    <div className="flex items-center gap-3 mb-4">
      ...
      />
      <div className="text-xl font-semibold text-gray-900 dark:text-white">Confluence Data Center</div>
    </div>
    <p className="text-base text-gray-700 dark:text-gray-300 mt-0 mb-5">Self-hosted team workspace and documentation platform</p>
...
- **Attachments and comments** of a restricted page are restricted the same way as the page. When the audit log reports a page becoming restricted, or opened up, its stored attachments and comments follow
- **Groups** — a restriction that names a group only by its name is matched by name. A page restricted to a group PipesHub hasn't synced yet stays restricted, and the group's members get access once the group syncs and the page is read again
- **Failed reads** — if a space's permissions, a group's members, or a page's restrictions can't be read for a temporary reason, PipesHub keeps what it already has. A page whose restrictions can't be read is skipped for that sync, while the rest of the space keeps syncing. The space's checkpoint doesn't move, so the next sync reads that window again and retries the page. A failed read never opens a page to its whole space and never removes access
