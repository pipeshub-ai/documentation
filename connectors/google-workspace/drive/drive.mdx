...
- A folder that was deleted, or that the connected account can't see, is left out, and the rest of the sync carries on.
- Any other problem, such as a Google outage or a quota limit that outlasts the retries, fails that sync run instead of quietly leaving the folder's files out. The next run starts from the same point, so no files are skipped for good.
