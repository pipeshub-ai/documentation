...
A full sync also replays past sharing events from Box's enterprise event history (Box keeps up to a year of it), so each person's **Shared with me** is complete. The replay saves its place after each page, as long as nothing in that full sync has failed to read so far. If something has, it still reads the pages but doesn't save its place, so the next full sync replays the same history:

- On a large enterprise, it reads a set number of pages per sync and carries on from where it stopped on the next one, so the whole history is applied within a few syncs of connecting.
