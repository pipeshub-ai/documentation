...
<Note>
  Sharing never moves a file. A file or folder owned by someone in your Box enterprise stays in its owner's drive, in its place in the owner's folders, and the colleagues it is shared with see it under **Shared with me**. Items owned by someone outside your enterprise are brought in only when a full sync finds them in a colleague's folders, and then appear only under **Shared with me**. A later share from outside the enterprise isn't brought in by the incremental sync, because Box's API answers "not found" when PipesHub asks for such a file as the colleague.
</Note>
