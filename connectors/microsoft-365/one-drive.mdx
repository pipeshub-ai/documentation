...
- **A folder that stops being shared:** files inside it whose own access can't be read are given the folder's current access, which also drops any sharing of their own. The sync logs name them so they can be reindexed.
