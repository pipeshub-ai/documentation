...
| Sign-in session expired | `404` | "Your sign-in session expired. Start again from the sign-in page." |

After five wrong passwords or codes in a row, the account is locked for 24 hours and the owner is emailed. During the lock, even correct credentials get the same answer as a wrong password or code. `404` from this endpoint now only means that the sign-in session expired, and it no longer returns `410` for an expired code.
</Tab>
</Tabs>
</Accordion>
