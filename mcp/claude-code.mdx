...
<Steps>
  <Step title="Mint a token">
    ...
    The token is sent as a bearer header on every request, so the URL must be `https://` for anything other than a local instance — `http://localhost:3000/mcp` is fine on your own machine, `http://` to a remote host is not.
