...
| Check | Expect |
| --- | --- |
| `pipeshub auth status --json` | Connected; your user and org; no token printed (if an error page or proxy echoes the token back, the CLI shows `[redacted]` in its place) |
...
| `could not reach …/mcp: fetch failed (…)` | Network, not auth. The part in brackets is the reason: `ECONNREFUSED` means nothing listens there, `ENOTFOUND` means the name does not resolve, a certificate message means this machine does not trust the instance's HTTPS certificate | Fix the address or the certificate; from a sandbox, use a public HTTPS origin |
| `PIPESHUB_BASE_URL is not a valid URL. It does not look like an address; if it is your token, it belongs in PIPESHUB_TOKEN.` (for a bare `eyJ…` token without the `phpat_` prefix: `…It holds what looks like a token, which belongs in PIPESHUB_TOKEN.`) | The two keychain entries are swapped. The CLI never prints the value | Put the PAT under `PIPESHUB_TOKEN` and the origin under `PIPESHUB_BASE_URL` |
| `auth status` says not connected, with a reason such as "token revoked" | The instance refused the token and said why | Mint a new PAT and replace the keychain entry |
...
| `The server at http://… redirected to https://…`, exit `2` | `PIPESHUB_BASE_URL` points at an address that redirects, usually `http://` to `https://`. The CLI does not follow it and does not send the token there | Set `PIPESHUB_BASE_URL` to the origin the message names. If it says the target "is not this instance's MCP endpoint", do not copy that address: use the one PipesHub itself answers on |
