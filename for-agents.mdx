...
Honest cost: on the order of **30–60 minutes**, a **16 GB-class** machine, then first-run in a **browser** (account + an LLM — vendor key in the form, or Ollama). Search 500s until that LLM exists. For data, the [Demo connector](/demo-data) loads a fictional company with nothing to connect (first release after 0.8.0); otherwise a Knowledge Base upload or Local FS — not Slack/Drive OAuth.
...
| 5. Data | Demo connector (`PIPESHUB_DEMO_DATA=1`, or enable it in the UI), else KB upload or Local FS mount | Nothing for the demo; otherwise drop files / pick a folder |
...
**Claude Code** with a personal access token (`claude mcp add` — not Cursor's `auth` JSON). `PIPESHUB_MCP_URL` and `PIPESHUB_MCP_TOKEN` are the two lines shown when the token is minted; the URL already ends in `/mcp`:
...
A project-scoped `.mcp.json` uses `type: "http"` with a `headers` entry for the same bearer token. Only the shared or admin-managed OAuth setup, below, uses an `oauth` object instead. See [Connect Claude Code](/mcp/claude-code).

**Claude Code with an OAuth app** — only for shared or admin-managed setups, where an admin created an [OAuth app](/mcp/overview#step-1-create-an-oauth-app):
