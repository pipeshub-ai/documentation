...
Click **Next** here you can setup your sync strategy either **Manual**, **Scheduled** or via **Webhooks** (to be implemented). On scheduled you can setup to run sync every 15 min, 30mins, 1hr, 4hr etc. And also decide batch size (this is an internal connector settings that decide how many items should the conenctor sync to ArangoDB at once) leave it empty if you want to go with default option 100.
...
1. Click **Save Configuration**. Now the authenticate button will be active, click on it and you will be taken to dropbox to authorize you app (make sure you are logged in to correct dropbox account).
