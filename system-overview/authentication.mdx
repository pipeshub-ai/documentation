...
- `HTTP_UNAUTHORIZED` - A missing, invalid or expired sign-in token or sign-in session; a failed sign-in-code check (wrong, expired or already-used code, no code requested, unknown email, or a locked account); or a sign-in step completed with a different account. A wrong password is `HTTP_BAD_REQUEST` instead
