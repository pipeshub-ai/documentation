...
Two more capabilities are attached only when the agent has no knowledge sources:
**Retrieval** and **Knowledge Hub**. When knowledge is active, Knowledge Graph covers
both searching and listing, so those two are left out rather than giving the model
two overlapping sets of tools.
