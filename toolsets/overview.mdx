...
These are the toolsets loaded by every PipesHub deployment. Tool counts come from the toolsets themselves.
...
That is **28 toolsets** and **365 tools**.

<Note>
The repository contains additional toolset implementations — Notion, Linear, GitLab, Dropbox, Box, S3, Zendesk and others — that are not loaded in this release. They are commented out of the toolset discovery list in `toolset_registry.py`, so they do not appear in **Actions** and cannot be added to an agent yet.
</Note>
