...
- **Deletions keep being retried:** A deleted file never changes again, so a deletion that still fails on the 5th sync goes into a list of pending deletions, saved together with the cursor. Every sync retries that list first, before reading new changes. A deletion leaves the list once it's applied, or when the file's record is already gone from PipesHub. Until then, the file stays searchable.
