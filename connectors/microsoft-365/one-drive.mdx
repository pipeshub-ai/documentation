...
PipesHub never treats a failed read as "no access" or "no members". If the sharing of a file PipesHub already has, or a group's member list, can't be read, the file or group keeps the access PipesHub already has, and a later sync catches up:

- **Temporary errors** (throttling, a 5xx error, a network problem) hold that page of changes for the next run, up to 5 runs. After that the page moves on, the item keeps its stored access, and the sync logs name it. Only the affected user's drive, or the group sync, waits; other users keep syncing.
- **Permanent refusals** (403 or 404) keep the stored access and move on straight away.
- **A new file whose sharing can't be read** is saved with no access once its page moves on (straight away for a 403 or 404). Its sharing is then read again on every later sync, with no limit, until it can be read.
