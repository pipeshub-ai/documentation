...
A grounded answer includes citations with `recordId` and `webUrl`. An `ask` with **no citations** means nothing was retrieved. Usually the documents genuinely do not contain it — say so. Sometimes the answer asserts facts with no citations; do not repeat them. Ignore `confidence` either way: it reads `Very High` on both. Do not invent a source.
