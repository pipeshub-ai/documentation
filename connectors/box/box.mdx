...
<Note>
  Sharing never moves a file. A file or folder owned by someone in your Box enterprise stays in its owner's drive, in its place in the owner's folders, and the colleagues it is shared with see it under **Shared with me**. A folder owned by someone outside your enterprise and shared with a colleague is brought in when a full sync finds it in that colleague's folders, and appears only under the colleague's **Shared with me**. A share whose owner is outside the enterprise isn't added any other way: incremental sync and the share-history replay skip it, because Box's API answers "not found" when PipesHub asks for such an item as the colleague. A single file shared this way never shows up in the colleague's folders, so it isn't brought in at all.
</Note>
