...
<Info>
  Some write or manage scopes (such as <code>write:jira-work</code> or <code>manage:jira-configuration</code>) may be required by Atlassian for certain API endpoints, even if the connector only performs read operations. The connector itself does <b>not</b> create, modify, or delete any Jira data. These permissions are requested solely to satisfy Atlassian's API requirements.
</Info>
...
5. Click **"Save Auth Settings"** to save auth setting
