...
<Info>
  SAML SSO can be turned on alongside other methods, such as Password or One-Time Password. The settings page saves every method you turn on as one sign-in step, and people choose any one of them on the login page.
</Info>

<Note>
  What SAML SSO can't do yet is be one of several sign-in **steps** (for example "SAML, then a code sent by email"). Sharing one step with other methods, as the settings page does, is fine. If a sign-in policy with more than one step includes SAML, saving it through the API is refused with *"SAML single sign-on can't be combined with other sign-in steps yet. Use SAML on its own as a one-step sign-in, or remove it from the policy."* Policies saved before this rule still sign people in, but a SAML sign-in completes them without the other steps, so change such a policy to a single step.
</Note>
...
| An app calling the API gets *"Single sign-on (SAML) can't be completed with this request"* | SAML works through browser redirects, so it can't be sent to `POST /api/v1/userAccount/authenticate`. Send the browser to `GET /api/v1/saml/signIn` instead |
