...
<Warning>
  **You cannot use OAuth 2.0 with Jira Data Center.** The OAuth apps you create in the Atlassian Developer Console only work with Jira Cloud.
</Warning>
...
No. OAuth apps created in the Atlassian Developer Console only work with Jira Cloud. This toolset supports **Personal Access Token** and **Basic Authentication** only.
...
<Info>
  When searching issues, always include a time range, such as "from the last 30 days" or "updated this week". Jira rejects searches that are not bounded by time.
</Info>
