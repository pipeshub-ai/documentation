...
A refused schedule leaves the connector's existing schedule running unchanged.
...
- Jobs are removed and recreated when updating schedules (no job modification). The new schedule is checked first, so a schedule that is refused never removes the one already in place
