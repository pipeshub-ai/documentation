...
- **Deletions keep being retried:** A deleted file never changes again, so a deletion that still fails on the 5th sync goes into a list of pending deletions, saved together with the cursor. A file that Nextcloud lists again during that sync (because it was restored or recreated) isn't queued. Every sync retries that list first, before reading new changes, and checks with Nextcloud before deleting:
  - if the file is gone (or a different file now sits at its path), the deletion is applied;
  - if the same file is back (it was restored), the deletion is dropped and the file stays;
  - if Nextcloud can't be checked right now, or PipesHub can't read the file's full stored location, the deletion stays on the list for the next sync.

  A deletion also leaves the list when nothing from that file or folder is left in PipesHub. If a folder's own record is already gone but files from inside it are still indexed, the connector checks the folder's old location in Nextcloud first. The files are removed only if the folder is really gone; if it was restored, they stay. The deletion stays on the list until that is settled. Until a deletion is applied, the file stays searchable.
