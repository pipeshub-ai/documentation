...
Ask both *"What is the enterprise pricing strategy for 2026?"* — Alice gets nothing relevant, Bob gets the document, cited from Google Drive. Everyone else, including the admin who set the demo up, sees what Alice sees, so for them that question comes back empty too. That is the permission check working, not a missing record.
...
<Warning>
  These are real accounts in your organisation with a password you chose. Keep the instance on localhost while they exist, or use them only on a throwaway instance; anyone who can reach the sign-in page and knows the password can sign in as Alice or Bob. Delete them (Workspace → Users) when you are done.
</Warning>

## Remove it

Remove the demo once you connect your own data. Until then, Acme's records stay in the answers of everyone in its groups — including the admin who set it up — right next to your company's real records, and each one is labelled with the system it imitates (Jira, Slack, Google Drive, …). Answers mark them with a **Demo** badge, and once your own data is indexed, PipesHub offers admins a **Remove demo data** button on the chat landing and the Connectors page, which also offers to delete Alice and Bob.
