...
- Syncs every attachment of a page or blog post, however many it has. If a later part of the attachment list can't be read, the attachments already read are kept, and the space's page or blog post checkpoint doesn't move, so the next sync reads that time window again. If even the first part can't be read, the connector uses the attachments that came with the page listing, and the checkpoint moves on as usual
