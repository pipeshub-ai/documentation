...
These are now read from the environment at startup. The installer writes them into
`deployment/docker-compose/.env`, and the Helm chart sets them from `values.yaml`:

| Setting | Environment variable |
|---------|----------------------|
| Graph database | `DATA_STORE` (`neo4j` by default, or `arangodb`) |
| Vector database | `VECTOR_DB_TYPE` (`qdrant` by default) |
| Key-value store | `KV_STORE_TYPE` (`redis` by default, or `etcd`) |
| Message broker | `MESSAGE_BROKER` (`redis` by default, or `kafka`) |
| MongoDB | `MONGO_URI`, `MONGO_USERNAME`, `MONGO_PASSWORD` |
