...
PipesHub stores the knowledge graph — the relationships between records, users, and metadata — in a graph database. Which one an instance uses is set by the `DATA_STORE` environment variable, and services talk to it through the `IGraphDBProvider` abstraction rather than a driver directly.
