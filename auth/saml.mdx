...
<Note>
  SAML SSO can't be combined with other sign-in steps yet (for example "SAML, then a code sent by email"). If a sign-in policy with more than one step includes SAML, saving it through the API is refused with *"SAML single sign-on can't be combined with other sign-in steps yet. Use SAML on its own as a one-step sign-in, or remove it from the policy."* Policies saved before this rule still sign people in, but a SAML sign-in completes them without the other steps, so change such a policy to SAML on its own.
</Note>
