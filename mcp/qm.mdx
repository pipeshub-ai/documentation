...
A grounded answer includes citations with `recordId` and `webUrl`. An `ask` with **no citations** means nothing was retrieved — often the correct answer: the documents do not contain it. Do not invent a source.
