...
PipesHub never treats a failed read as "no access" or "no members". If a file's sharing or a group's member list can't be read, the file or group keeps the access PipesHub already has, and a later sync catches up:

- **Temporary errors** (throttling, a 5xx error, a network problem) hold that part of the sync for the next run, up to 5 runs. After that the item is skipped, keeping its stored access, and the sync logs name it so it can be reindexed. Only the affected user's drive, or the group sync, waits; other users keep syncing.
